...
from .consts import LOGICAL_TIMESTAMP_VARNAME, MSG_SRCNODE_VARNAME, RECEIVED_FUNNAME, SENT_FUNNAME, is_use_set_for_queue
...
class SentReceivedTransformer(NodeTransformer):
    ...
    def _visit_call_main(self, event):
        ...
        if (same == None):
            ...
            self.info.membervars[event.vname] = "set" #FIXME: set or list??
    ...
    def genVarAssign(self, vname):
        return Assign([Attribute(Name("self", Load()), vname, Store())],
                      Call(Name("set", Load()) if is_use_set_for_queue() else
                           Name("list", Load()),
                           [], [], None, None))
    ...
    def genHandlerFuncDef(self, name, vname, vlist):
        ...
        m = Attribute(Name(vname, Load()),
                      "add" if is_use_set_for_queue() else "append",
                      Load())
        if (len(vlist) > 0):
    ...
    def genPredicateFuncDef(self, name, vname, vlist, bound, free):
        ...
        listnode = SetComp(elt, [comprehension(target, iterator, ifs)])
                    # if is_use_set_for_queue() else
                    # ListComp(elt, [comprehension(target, iterator, ifs)]))

        retstmt = Return(listnode)

        self.info.evtpreds[name] = listnode
