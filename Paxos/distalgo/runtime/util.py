...
def dist_source(dir, filename):
    ...
    if filename.endswith(".dis") or filename.endswith(".da"):
...
def use_channel(endpoint):
    ...
    if RootProcess is not None:
        if EndPointType != ept:
            Log.warn("Can not change channel type after creating child processes.")
        return
    EndPointType = ept
...
def entrypoint(options, args, cmdl):
    ...
    # Start main program
    if hasattr(module, 'main') and isinstance(module.main, types.FunctionType):
